...
const Movie = () => {
    ...
    const { data , loading , error} = useSelector(state => state.getActorMovies)

    useEffect(()=> {
        window.scroll({top : 0 , behavior : 'smooth'})
        dispatch(getAsyncActorMoviesAction(actorId))
    },[actorId])
    ...
    return (
        <div className={Styles.parent}>
           ...
           { data && data.length > 0 &&  <h1 className={Styles.title}>Movies :</h1>}
